...
'''
	...
	This modules provides a class `DataSender` that allows users to
	easily transfer data over an IEEE 802.15.4 link, using digi XBee modules.XBee
	...
	The DataSender class is a Singleton (because of the shared XBeeDevice resource),
	so all instances are in fact the same. Be carefull, arguments given to the constructor
	are only used on the first 'instanciation. A user can 'instanciate' one like a normal
	object, but it will always be the same. This class is also a Process that starts on
	creation, in order to be able to notify data/file availability from the main Process
	without being blocked while trying to send them.
...
from econect.protocol.I8DP import (I8DP_Trame, i8dp_build_ack_trame,
                                   i8dp_build_data_trame, i8dp_send_trame)
from econect.protocol.I8RP import I8RP_Trame, i8rp_request
from econect.protocol.I8TP import (I8TP_Trame, i8tp_build_response,
                                   i8tp_request_delta)


def i8tl_send_trame(device : XBeeDevice, destination_addr : Union[XBee16BitAddress,XBee64BitAddress], data : bytes, logger : logging.Logger) -> None:
	try:
		...
		if destination_addr is None:
			logger.error('Address was None. Could not send data.')
		elif data is None:
			logger.error('Trame was None. Could not send data.')
		logger.error(f'{ve}')
		...
		logger.error('Device should be in API mode but is not.')
		logger.error(f'{iome}')
	except TransmitException as te:
		logger.error(f'{te}')
	except XBeeException as xe:
		logger.error('Device communication interface is closed (maybe unplugged?).')
		logger.error(f'{xe}')
...
def _get_chunk_count(filename : str) -> int:
	...
	return (size + I8DP_Trame.PAYLOAD_MAX_LEN - 1)//I8DP_Trame.PAYLOAD_MAX_LEN



class DataSender(metaclass=Singleton):
	__slots__ = ('_process', '_device', '_coord_addr', '_queue', '_stop_event', '_xbee_init_event', '_tmp_dir', '_logger', '_del_dir', '_retries', '_response_timeout', '_timestamp_delta', '_delta_lifetime' )
	...
	def __init__(self, 
		...
		self._logger = logging.getLogger('DataSender')
	...
	def _init_device_coord_addr_and_timestamp_delta(self,  path: str, speed: int, coord_addr : XBee64BitAddress):	
		...
		self._logger.info(f'Own device 16 bits address: {self._device.get_16bit_addr()}')
		self._logger.info(f'Own device 64 bits address: {self._device.get_64bit_addr()}')
		...
		while not self._coord_addr and not self._stop_event.is_set()	:
			self._coord_addr = i8rp_request(self._device)
			if self._coord_addr is None:
				self._logger.warning(f'Could not get IEEE 802.15.4 Coordinator 64 bits address (try {i}).')
				i+= 1

		if self._stop_event.is_set():
			return

		self._logger.info(f'Coordinator 64 bits address: {self._coord_addr}')
	...
	def _update_timestamp_delta(self, validity_in_hours : int=0):
		delta_list = []
		for _ in range(10):
			delta_list.append(i8tp_request_delta(self._device, self._coord_addr, timeout=self._response_timeout))
		...
		self._logger.info(f'Got new timestamp_delta ({self._timestamp_delta}), valid until {datetime.fromtimestamp(self._delta_lifetime*1e-9)}')
	

	def stop(self) -> None:
		self._logger.info("Exiting. Waiting for DataSender process to stop.")
		self._stop_event.set()
		self._process.join()



	# TODO: Need a flag in option to say "it's the first one so we can cancel a send and not fuck everything up."
	def _resend_chunks(self, trames_to_resend : List[I8DP_Trame]) -> bool:
		self._logger.info(f'Trying to resend {len(trames_to_resend)} trames.')
		while len(trames_to_resend) > 0:
			...
			while not ack and tries <= self._retries:
				self._logger.info(f'Resending trame {a_trame.seq}. (try {tries}/{self._retries})')
				i8dp_ack = i8dp_send_trame(self._device, self._coord_addr, a_trame, timeout=self._response_timeout)
				if i8dp_ack is not None:
					ack_list = [i8dp_ack.seq] + i8dp_ack.ack_list
					trames_to_remove = list(set(ack_list) & set(trames_to_resend))
					self._logger.info(f'Trames {trames_to_remove} correctly acknowledged.')
					ack = True
					for a_trame_to_remove in trames_to_remove:
						try:
							trames_to_resend.remove(a_trame_to_remove)
						except ValueError as ve:
							self._logger.error(f'Could not remove {a_trame_to_remove}, not in {trames_to_resend} ({ve})')
				else: 
					self._logger.warning(f'Acknowledgment not received for trame {a_trame.seq}.')
				tries +=1
	...
	def _send_file(self, file : IO[bytes], chunk_count : int) -> bool:
		...
		reset = False
		while not reset:
			i8dp_rst_ack = i8dp_send_trame(self._device, self._coord_addr, I8DP_Trame.rst_trame(), timeout=self._response_timeout)
		...
		for i, chunk in enumerate(iter(partial(file.read, I8DP_Trame.PAYLOAD_MAX_LEN), b''), 1):
			...
			trame = i8dp_build_data_trame(chunk, begin=(i == 1), need_ack=((i%I8DP_Trame.BURST_MAX_LEN == 0) or (i == chunk_count)), more_fragments=(i != chunk_count))

			if trame.seq == (I8DP_Trame.SequenceGenerator.LIMIT - 1):
				trame.set_needs_ack()

			
			self._logger.info(f'Trying to send chunk {i} of {chunk_count} with seq: {trame.seq}, need_ack:{trame.needs_ack} and more_fragments:{trame.more_frag}')
			i8dp_ack = i8dp_send_trame(self._device, self._coord_addr, trame, timeout=self._response_timeout)
			...
			if not trame.needs_ack:
				...
				if i8dp_ack is None:
					...
					self._logger.warning(f'ACK for trame {trame.seq} and {len(trames_sent)} previous trames is needed but was not received.')
				else:
					...
					if to_resend:
						self._logger.warning(f'ACK for trame {trame.seq} and {len(trames_sent)} previous trames is needed but was partially received ({len(to_resend)} trames were not ACKd)')
					else:
						self._logger.info(f'ACK for trame {trame.seq} and {len(trames_sent)} previous trames is needed  and was succesfuly received.')
	...
	def __run(self, path : str, speed : int, coord_addr : XBee64BitAddress):
		if multiprocessing.parent_process() is None:
			self._logger.error("Tried to call run() method from main process.")
			return
		
		self._logger.info("DataSender process created")
		...
		while not self._stop_event.is_set():
			...
			try:
				...
				self._logger.info(f'To send: {filename}')
				...
				if file_sent:	
					self._logger.info(f'Sent file {filename}')
					remove(filename)
				
				else:
					self._logger.warning(f'Could not send {filename}, putting it back into the queue.')
					self._queue.put(filename)
			except queue.Empty:
				pass
				
		self._logger.info("DataSender process exiting")
	...
	def notify_data_to_send(self, data: bytes) -> None:
		...
		self._queue.put(filename)
		self._logger.info(f'Notify:  {filename}')
...
class DataReceiver(metaclass=Singleton):
	...
	class ReceiverThreadPool():
		__slots__ = ('_tmp_dir', '_inactive_time_limit', '_threads', '_thread_stop_events', '_thread_trame_reception_queues', '_assembled_data_queue', '_notify_lock', '_responses_trames_queue', '_ack_thread_stop_event', '_ack_thread')
		...
		def __run_send_responses(device : XBeeDevice, stop_event : threading.Event, responses_trames_queue : queue.Queue):
			...
			while not stop_event.is_set():
				try:
					...
					i8tl_send_trame(device, i8tl['to'], i8tl['trame'].to_bytes(), logger)
				...
				try:
					...
					try:
						...
						if   protocol_id == Protocol_ID.I8RP:
								logger.info("[I8RP] Received trame. Answering request.")
							...
							if i8tp_trame.is_req:
								logger.info("[I8TP] Answering request.")
								responses_trames_queue.put({'trame' : i8tp_build_response(), 'to' : data['from']})
							...
							if i8dp_trame.is_rst:
								...
								if transmission_in_progress and i8dp_trame.needs_ack:
									...
									responses_trames_queue.put({'trame' : i8dp_build_ack_trame(i8dp_trame.seq, seq_ack_list), 'to' : data['from']})
									seq_ack_list = []
	...
	__slots__ = ('_process', '_device', '_queue', '_stop_event', '_tmp_dir', '_logger', '_del_dir', '_thread_inactive_time_limit')
	...
	def __init__(self, 
		...
		self._logger = logging.getLogger('DataReceiver')
	...
	def stop(self) -> None:
		self._logger.info("Stopping DataReceiver process")
		self._stop_event.set()
		self._process.join()
	...
	def __run(self, path : str, speed : int):	
		...
		self._logger.info("DataReceiver process created")
